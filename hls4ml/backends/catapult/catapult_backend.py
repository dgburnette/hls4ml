...
class CatapultBackend(FPGABackend):
    ...
    def _register_layer_attributes(self):
        ...
        for layer in cnn_layers:
            ...
            attrs.append(
                ChoiceAttribute(
                    'conv_implementation',
                    choices=['LineBuffer', 'Encoded', 'ac_window'],
                    default='LineBuffer',
    ...
    def _register_flows(self):
        ...
        optimization_passes = [
            ...
            'catapult:distributed_arithmetic_codegen',
            'catapult:distributed_arithmetic_einsum_codegen',
            'catapult:fuse_quantizer_into_d_a_layers',
    ...
    def create_initial_config(
        ...
        **_,
        ...
        """
        print("Executing create_initial_config")
        config = {}
        ...
        config['MemType'] = None
        config['ParamStore'] = ParamStore
        if ParamStore == 'inline':
            config['ROMLocation'] = 'Local'
            config['MemType'] = None
        elif ParamStore == 'interface':
            config['ROMLocation'] = 'Local'
            config['MemType'] = None
        elif ParamStore == 'merged':
            config['ROMLocation'] = 'Local'
            config['MemType'] = 'RAM'
        else:
            config['ROMLocation'] = 'Local'
        ...
        config['GlobalDirectives'] = {
            'analyze': {
                ...
                'MEM_MAP_THRESHOLD': '[expr 2048 * 16 + 1]',
                'REDUNDANT_RTL_ELIMINATION': 'false',
            },
            'compile': {
            },
            'libraries': {},
            'assembly': {
                'ADVANCED_DPOPT': 'false',
            },
            'architect': {},
            'allocate': {},
            'schedule': {},
    ...
    def build(
        ...
        print(ccs_invoke)
        if Silent is True:
            print('Running Catapult in batch mode - transcript suppressed')
            os.system(f'{ccs_invoke} >/dev/null')
        else:
    ...
    def init_mha(self, layer):
        ...
        layer.set_attr('strategy', 'latency')  # resource
