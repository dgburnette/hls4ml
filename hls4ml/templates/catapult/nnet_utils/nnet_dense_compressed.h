                      ...
                      typename CONFIG_T::bias_t biases[CONFIG_T::n_out])
{
    ...
    #pragma hls_unroll
    InitAccum: for (unsigned i = 0; i < CONFIG_T::n_out; i++) {
        acc[i] = (typename CONFIG_T::accum_t)(biases[i]);
    }

    // Do the compressed matrix-multiply
    const int rufactor = CONFIG_T::reuse_factor;
    ReuseLoop: for (unsigned ir = 0; ir < rufactor; ir++) {
        ...
        #pragma hls_unroll
        CompressedMultLoop: for (unsigned im = 0; im < multiplier_limit; im++) {
    ...
    // Cast to "res_t" type
    #pragma hls_unroll
    ResultLoop: for (unsigned i = 0; i < CONFIG_T::n_out; i++) {
        // res[i] = (res_T) (acc[i]);
...
template <class data_T, class res_T, typename CONFIG_T>
void dense_compressed(data_T data[CONFIG_T::n_in], ac_sync &sync_data,
                      res_T res[CONFIG_T::n_out], ac_sync &sync_res,
                      typename CONFIG_T::weight_t weights[CONFIG_T::n_nonzeros],
                      typename CONFIG_T::bias_t biases[CONFIG_T::n_out])
{
  sync_data.sync_in();
  dense_compressed<data_T, res_T, CONFIG_T>(data, res, weights, biases);
  sync_res.sync_out();
}
