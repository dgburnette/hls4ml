...
template <typename T, int N, typename accum_t> 
accum_t max(T x[N]) 
{
    T y = x[0];
    for (int i = 1; i < N; i++) {
...
template <typename T, int N, typename accum_t> 
accum_t avg(T (&x)[N], unsigned length) 
{
    accum_t y = 0;
    for (int i = 0; i < N; i++) {
        y += x[i];
...
template <typename T, int N, Pool_Op op, typename accum_t> 
accum_t pool_op(T (&x)[N], unsigned length) 
{
    switch (op) {
    case Max:
...
template <class data_T, class res_T, typename CONFIG_T>
void pooling1d_cl(data_T data[CONFIG_T::n_in * CONFIG_T::n_filt], ac_sync &sync_data,
                  res_T res[CONFIG_T::n_out * CONFIG_T::n_filt], ac_sync &sync_res) 
{
  sync_data.sync_in();
  pooling1d_cl<data_T, res_T, CONFIG_T>(data, res);
  sync_res.sync_out();
}
...
#pragma hls_design block
template <class data_T, class res_T, typename CONFIG_T>
void global_pooling1d_cl(data_T data[CONFIG_T::n_in * CONFIG_T::n_filt], ac_sync &sync_data,
                         res_T res[CONFIG_T::n_filt], ac_sync &sync_res) 
{
  sync_data.sync_in();
  global_pooling1d_cl<data_T, res_T, CONFIG_T>(data, res);
  sync_res.sync_out();
}
...
#pragma hls_design block
template <class data_T, class res_T, typename CONFIG_T>
void pooling2d_cl(data_T data[CONFIG_T::in_height * CONFIG_T::in_width * CONFIG_T::n_filt], ac_sync &sync_data,
                  res_T res[CONFIG_T::out_height * CONFIG_T::out_width * CONFIG_T::n_filt], ac_sync &sync_res) 
{
  sync_data.sync_in();
  pooling2d_cl<data_T, res_T, CONFIG_T>(data, res);
  sync_res.sync_out();
}
...
#pragma hls_design block
template <class data_T, class res_T, typename CONFIG_T>
void pooling2d_cf(data_T data[CONFIG_T::in_height * CONFIG_T::in_width * CONFIG_T::n_filt], ac_sync &sync_data,
                  res_T res[CONFIG_T::out_height * CONFIG_T::out_width * CONFIG_T::n_filt], ac_sync &sync_res) 
{
  sync_data.sync_in();
  pooling2d_cf<data_T, res_T, CONFIG_T>(data, res);
  sync_res.sync_out();
}
                         ...
                         res_T res[CONFIG_T::n_filt]) 
{
    ...
    FiltLoop: for (int filt = 0; filt < CONFIG_T::n_filt; filt++) {
        data_T pool[CONFIG_T::in_height * CONFIG_T::in_width];

        InputLoop: for (int i = 0; i < CONFIG_T::in_height * CONFIG_T::in_width; i++) {
...
#pragma hls_design block
template <class data_T, class res_T, typename CONFIG_T>
void global_pooling2d_cl(data_T data[CONFIG_T::in_height * CONFIG_T::in_width * CONFIG_T::n_filt], ac_sync &sync_data,
                         res_T res[CONFIG_T::n_filt], ac_sync &sync_res) 
{
  sync_data.sync_in();
  global_pooling2d_cl<data_T, res_T,CONFIG_T>(data, res);
  sync_res.sync_out();
}
