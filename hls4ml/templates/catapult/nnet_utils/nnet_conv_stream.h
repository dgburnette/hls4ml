...
enum class conv_implementation { linebuffer = 0, encoded = 1 , ac_window = 2 };
enum class padding_type { valid = 0, same = 1 };
                     ...
                     typename data_T::value_type kernel_window[CONFIG_T::filt_width * CONFIG_T::n_chan]) 
{
    ...
    #pragma hls_unroll
    KernelPushChannel: for (unsigned int i_ic = 0; i_ic < CONFIG_T::n_chan; i_ic++) {
    ...
    typename data_T::value_type kernel_window[CONFIG_T::filt_width * CONFIG_T::filt_height * CONFIG_T::n_chan]) 
{
    ...
    static const int lastheight = (CONFIG_T::filt_width - 1) * CONFIG_T::n_chan;
    #pragma hls_unroll
    KernelPushHeight: for (unsigned int i_ih = 0; i_ih < CONFIG_T::filt_height; i_ih++) {
        #pragma hls_unroll
        KernelPushChannel: for (unsigned int i_ic = 0; i_ic < CONFIG_T::n_chan; i_ic++) {
    ...
    typename data_T::value_type kernel_window[CONFIG_T::filt_height * CONFIG_T::filt_width * CONFIG_T::n_chan]) 
{
    ...
    #pragma hls_unroll
    UpdateBuffer: for (unsigned int i_ic = 0; i_ic < CONFIG_T::n_chan; i_ic++) {
        // Insert pixel(s) at end of shift buffer
        shift_buffer[CONFIG_T::filt_height - 1][i_ic] = in_elem[i_ic];
    }

    #pragma hls_unroll
    LineBufferDataIn: for (unsigned int i_ic = 0; i_ic < CONFIG_T::n_chan; i_ic++) {
