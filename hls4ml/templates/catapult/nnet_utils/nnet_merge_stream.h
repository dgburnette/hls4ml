...
{
    ConcatLoopHeight1: for (unsigned int i = 0; i < CONFIG_T::n_elem1_0; i++) {
        #pragma hls_pipeline_init_interval 1
        ConcatLoopWidth1: for (unsigned int j = 0; j < CONFIG_T::n_elem1_1; j++) {
            input1_T in_data1 = data1.read();
            res_T out_data;

            #pragma hls_unroll
            ConcatPackInput1: for (unsigned int k = 0; k < input1_T::size; k++) {
    ...
    ConcatLoopHeight2: for (unsigned int i = 0; i < CONFIG_T::n_elem2_0; i++) {
        #pragma hls_pipeline_init_interval 1
        ConcatLoopWidth2: for (unsigned int j = 0; j < CONFIG_T::n_elem2_1; j++) {
            input2_T in_data2 = data2.read();
            res_T out_data;

            #pragma hls_unroll
            ConcatPackInput2: for (unsigned int k = 0; k < input2_T::size; k++) {
...
{
    ConcatLoopHeight: for (unsigned int i = 0; i < CONFIG_T::n_elem1_0; i++) {
        #pragma hls_pipeline_init_interval 1
        ConcatLoopWidth1: for (unsigned int j = 0; j < CONFIG_T::n_elem1_1; j++) {
...
void concatenate3d_2(ac_channel<input1_T> &data1, ac_channel<input2_T> &data2, ac_channel<res_T> &res) 
{
    ConcatLoopHeight: for (unsigned int i = 0; i < CONFIG_T::n_elem1_0; i++) {
        #pragma hls_pipeline_init_interval 1
        ConcatLoopWidth: for (unsigned int j = 0; j < CONFIG_T::n_elem1_1; j++) {
