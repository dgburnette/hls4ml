...
void depthwise_conv_2d_latency_cl(
    ...
    const typename CONFIG_T::weight_t weights[CONFIG_T::filt_height * CONFIG_T::filt_width * CONFIG_T::n_filt],
    const typename CONFIG_T::bias_t biases[CONFIG_T::n_filt]) 
{
    ...
    constexpr unsigned mult_n_mult = mult_n_acc * mult_n_out;
    constexpr int ce_reuse_factor = CONFIG_T::reuse_factor;

    data_T data_buf[CONFIG_T::n_pixels][mult_n_in];

    typename CONFIG_T::accum_t mult[mult_n_mult];
    ...
    PartitionLoop: for (int i_part = 0; i_part < CONFIG_T::n_partitions; i_part++) {
        ...
        PixelLoop: for (unsigned i_pxl = 0; i_pxl < CONFIG_T::n_pixels; i_pxl++) {
            ...
            #pragma hls_unroll
            Product: for (int i_in = 0; i_in < mult_n_in; i_in++) {
                cache = data_buf[i_pxl][i_in]; 
                #pragma hls_unroll
                D_MULT: for (int i_dm = 0; i_dm < d_mult; i_dm++) {
                    mult[i_dm + d_mult*i_in] =
                        CONFIG_T::mult_config::template product<data_T, typename CONFIG_T::mult_config::weight_t>::product(
                            cache, weights[i_dm + d_mult*i_in]);
                }
            }
            ...
            #pragma hls_unroll
            Accum1: for (int i_in = 0; i_in < mult_n_acc; i_in++) {
                #pragma hls_unroll
